...
export class OrderProductStore {
	...
	async getID(order_id: string, product_id: string): Promise<Number> {
		try {
			...
			throw new Error(
				`Could not get id for order_product with product_id ${product_id} and order_id ${order_id}. Error: ${error}`
			);
		}
	}
	...
	async getCurrentQuantity(id: string): Promise<number> {
		try {
			...
			throw new Error(
				`Could not get quantity for order_product with id ${id}. Error: ${error}`
			);
		}
	}
	...
	async updateQuantity(id: string, newQuantity: string): Promise<OrderProduct> {
		try {
			...
			throw new Error(
				`Could not update quantity for order_product with id ${id}. Error: ${error}`
			);
		}
	}
}
